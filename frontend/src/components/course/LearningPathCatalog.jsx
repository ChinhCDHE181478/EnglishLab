...
export default function LearningPathCatalog({ courses = [] }) {
  ...
  const paths = useMemo(() => Object.values(courses.reduce((groups, course) => {
    ...
    return groups;
  }, {})).map((path) => ({ ...path, courses: path.courses.sort((left, right) => Number(left.learningPathOrder || 0) - Number(right.learningPathOrder || 0)) })), [courses]);
