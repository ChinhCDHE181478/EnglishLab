...
const CoursePurchaseCard = ({ course, isAuthenticated, purchasing, onPurchase }) => {
  ...
  return (
    <aside className="sticky top-24 rounded-[28px] border border-[#dfbfbd]/30 bg-white p-6 shadow-[0_24px_60px_rgba(43,40,40,0.08)]">
      ...
      {course.registered ? (
        <Link className="mt-6 flex w-full items-center justify-center rounded-2xl bg-[#8a0018] px-5 py-4 text-center text-sm font-extrabold text-white transition-all hover:-translate-y-0.5 hover:bg-[#650012]" to={`/courses/${course.slug}/learn`} state={{ course, workspaceMode: 'learn' }}>
          Đến khóa học
        </Link>
      ) : isAuthenticated ? (
        <button
