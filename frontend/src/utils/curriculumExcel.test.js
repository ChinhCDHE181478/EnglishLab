...
const headers = ['Tên khóa học', 'Tên Unit', 'Mô tả Unit', 'Thứ tự bài học', 'Tên bài học', 'Mô tả bài học', 'Mục tiêu học tập'];

describe('instructor-led course Excel import', () => {
  it('groups multiple lessons under the same Unit', () => {
  ...
  it('reports the exact row when a lesson number is duplicated', () => {
    expect(() => parseInstructorLedCourseExcelRows([
      headers,
      ['IELTS Reading', 'Unit 1', '', '1', 'Skimming', '', ''],
      ['', 'Unit 2', '', '1', 'Scanning', '', ''],
    ])).toThrow('Dòng 3: bài học 1 bị trùng');
  });

  it('creates each grouped Unit once before creating its lessons', async () => {
