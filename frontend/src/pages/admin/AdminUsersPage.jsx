...
export default function AdminUsersPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setError(requestError.response?.data?.message || 'Không tải được người dùng.');
    }
  }, [keyword, role, page]);
  ...
  const save = async (event) => {
    ...
    try {
      ...
      setError(requestError.response?.data?.message || 'Không lưu được người dùng.');
    }
  };
  ...
  const toggleStatus = async (user) => {
    try {
      ...
      setError(requestError.response?.data?.message || 'Không đổi được trạng thái.');
    }
  };

  const resendTeacherEmail = async (user) => {
    setError('');
    try {
      await adminApi.resendTeacherOnboardingEmail(user.id);
      window.alert(`Đã gửi lại email thiết lập tới ${user.email}.`);
    } catch (requestError) {
      setError(requestError.response?.data?.message || 'Không gửi lại được email thiết lập giáo viên.');
    }
  };

  const creatingTeacher = modal === 'create' && form.roles.includes('TEACHER');

  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <p className="text-xs font-bold uppercase tracking-[.18em] text-[#8a0018]">Phân quyền</p>
          <h1 className="mt-2 font-['Manrope'] text-3xl font-extrabold tracking-tight text-slate-900">Người dùng & vai trò</h1>
          <p className="mt-2 text-sm text-slate-500">Quản lý trạng thái tài khoản và vai trò trong hệ thống.</p>
        </div>
        <button className="inline-flex items-center justify-center gap-2 rounded-lg bg-[#4b0009] px-5 py-3 text-sm font-bold text-white shadow-sm hover:bg-[#730014]" onClick={openCreate} type="button">
      ...
      <ManagerFilterBar>
        <label className="flex min-w-[260px] flex-1 items-center gap-2 rounded-lg border border-[#dcc0bf]/40 bg-[#fffdfc] px-3">
          ...
          <input className="w-full bg-transparent py-2.5 text-sm outline-none" onChange={(event) => { setKeyword(event.target.value); setPage(0); }} placeholder="Tìm tên, email, số điện thoại" value={keyword} />
        </label>
        <BrandedSelect
          ...
          options={[{ label: 'Tất cả vai trò', value: '' }, ...ROLES.map((item) => ({ label: ROLE_LABELS[item], value: item }))]}
          value={role}
        />
      </ManagerFilterBar>
      ...
      <section className="overflow-hidden rounded-xl border border-[#dcc0bf]/30 bg-white shadow-sm">
        <ManagerTable columns={[{ label: 'Người dùng' }, { label: 'Vai trò' }, { label: 'Hồ sơ' }, { label: 'Trạng thái' }, { label: 'Thao tác', align: 'right' }]} minWidth="1000px">
          {users.map((user) => (
        ...
        <div className="border-t border-[#dcc0bf]/20 bg-[#eff4ff]/30 px-6 py-4">
          <Pagination onChange={(newPage) => setPage(newPage - 1)} page={page + 1} pageSize={15} totalPages={totalPages} />
        </div>
      </section>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#1a0004]/45 p-4">
          <form className="w-full max-w-2xl rounded-3xl border border-[#ead9db] bg-white p-6 shadow-2xl" onSubmit={save}>
            ...
            <div className="mt-6 flex justify-end gap-3">
              <button className="rounded-xl border border-[#dfbfbd] px-5 py-2.5 font-bold text-[#730014]" onClick={() => setModal(null)} type="button">Hủy</button>
              <button className="rounded-xl bg-[#4b0009] px-5 py-2.5 font-bold text-white disabled:opacity-50" disabled={!form.roles.length} type="submit">Lưu</button>
            </div>
          </form>
        </div>
      ) : null}
