...
export default function AdminLayout() {
  ...
  return <div className="min-h-screen bg-[#f8fafc] font-['Inter'] text-slate-800 lg:pl-[270px]">
    ...
    <header className="sticky top-0 z-20 flex min-h-[82px] items-center justify-between border-b border-slate-200 bg-white/95 px-5 py-4 backdrop-blur lg:px-8"><div className="flex items-center gap-3"><span className="flex h-10 w-10 items-center justify-center rounded-xl bg-[#730014] text-white"><Shield className="h-5 w-5" /></span><div><p className="font-extrabold text-slate-900">Quản trị hệ thống</p><p className="text-xs text-slate-500">Quản lý tài khoản, phân quyền và cấu hình</p></div></div><div className="hidden rounded-xl border border-slate-200 bg-white px-3 py-2 text-right shadow-sm sm:block"><p className="text-sm font-bold text-slate-900">{user?.fullName || user?.email}</p><p className="text-[10px] font-bold uppercase tracking-wider text-[#8a0018]">Administrator</p></div><nav className="flex gap-2 lg:hidden">{items.map(({ to, label, end }) => <NavLink className={({isActive}) => `rounded-lg px-3 py-2 text-xs font-bold ${isActive ? 'bg-[#730014] text-white' : 'text-slate-600'}`} end={end} key={to} to={to}>{label}</NavLink>)}</nav></header>
