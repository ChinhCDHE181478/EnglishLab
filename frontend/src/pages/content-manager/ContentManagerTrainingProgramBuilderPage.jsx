...
export default function ContentManagerTrainingProgramBuilderPage({ mode = 'OFFLINE' }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        if (active) setError(err?.response?.data?.message || 'Không tải được chương trình đào tạo.');
      } finally {
  ...
  const save = async () => {
    ...
    if (!form.curriculumProgramId) {
      setError('Vui lòng chọn giáo trình được sử dụng.');
      return;
    }
    setSaving(true);
    setError('');
    ...
    try {
      ...
      setSuccess(isNew ? 'Đã tạo chương trình đào tạo.' : 'Đã lưu chương trình đào tạo.');
      if (isNew) navigate(`${basePath}/${saved.id}/builder`, { replace: true });
    } catch (err) {
      setError(err?.response?.data?.message || 'Không lưu được chương trình đào tạo.');
    } finally {
      setSaving(false);
    }
  };

  if (loading) return <ContentManagerLoadingState message="Đang mở chương trình đào tạo..." />;
    ...
    <div className="space-y-5">
      <section className="rounded-2xl border border-[#dcc0bf]/35 bg-white p-5 shadow-sm">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            <Link className="inline-flex items-center gap-2 text-sm font-bold text-[#730014]" to={basePath}><ArrowLeft className="h-4 w-4" />Danh sách chương trình</Link>
            <p className="mt-4 text-xs font-extrabold uppercase tracking-[0.16em] text-[#8b706e]">{mode === 'VIRTUAL' ? 'Chương trình virtual' : 'Chương trình offline'}</p>
            <h1 className="mt-1 font-['Manrope'] text-2xl font-black text-[#0b1c30]">{isNew ? 'Tạo chương trình đào tạo' : form.title}</h1>
            <p className="mt-2 text-sm text-[#584140]">Đóng gói một giáo trình đã biên soạn thành sản phẩm đào tạo để Training Manager mở lớp.</p>
          </div>
          <button className={PRIMARY_BUTTON_CLASS} disabled={saving} onClick={save} type="button"><Save className="h-4 w-4" />{saving ? 'Đang lưu...' : isNew ? 'Tạo chương trình' : 'Lưu thay đổi'}</button>
        </div>
      </section>
      ...
      <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_360px]">
        <div className="space-y-5">
          <BuilderSection number="01" title="Nhận diện và giáo trình sử dụng">
            <div className="grid gap-4 md:grid-cols-2">
              <TextInput label="Tên chương trình" value={form.title} onChange={(value) => updateForm({ title: value, code: form.code || makeCode(value, mode), slug: form.slug || toSlug(value) })} />
              <div><FieldLabel>Giáo trình sử dụng</FieldLabel><BrandedSelect onChange={(event) => updateForm({ curriculumProgramId: event.target.value })} options={matchingCurriculums.map((item) => ({ label: item.title, value: String(item.id), description: [item.code, item.examCategory, item.status, `${item.totalUnits || 0} unit`].filter(Boolean).join(' · ') }))} placeholder="Chọn giáo trình" searchable={true} value={form.curriculumProgramId} /></div>
              <TextInput label="Mã chương trình" value={form.code} onChange={(value) => updateForm({ code: value })} />
          ...
          <BuilderSection number="02" title="Đóng gói triển khai và thương mại">
            <div className="grid gap-4 md:grid-cols-2">
              ...
              <label className="flex items-center gap-3 rounded-xl border border-[#dcc0bf]/35 bg-[#fcfbfb] px-4 py-3 text-sm font-bold text-[#584140]"><input checked={form.featured} className="h-4 w-4 accent-[#730014]" onChange={(event) => updateForm({ featured: event.target.checked })} type="checkbox" />Đánh dấu chương trình nổi bật</label>
            </div>
          </BuilderSection>
          ...
          <BuilderSection number="03" title="Thông tin giới thiệu">
            <div className="grid gap-4">
              ...
              <TextArea label="Mô tả chương trình" value={form.description} onChange={(value) => updateForm({ description: value })} />
              <TextInput label="Ảnh đại diện" placeholder="URL ảnh đại diện chương trình" value={form.thumbnailUrl} onChange={(value) => updateForm({ thumbnailUrl: value })} />
            </div>
          </BuilderSection>
        </div>
        ...
        <aside className="h-fit space-y-4 xl:sticky xl:top-24">
          <section className="rounded-2xl border border-[#dcc0bf]/35 bg-white p-5 shadow-sm">
            <div className="flex items-center gap-3 border-b border-[#f0e4e5] pb-4"><BookOpenCheck className="h-5 w-5 text-[#730014]" /><div><p className="text-xs font-extrabold uppercase tracking-[0.14em] text-[#8b706e]">Nguồn nội dung</p><h2 className="mt-1 font-['Manrope'] text-lg font-black text-[#0b1c30]">Giáo trình được sử dụng</h2></div></div>
            {selectedCurriculum ? <div className="mt-4 space-y-3"><p className="font-extrabold text-[#0b1c30]">{selectedCurriculum.title}</p><SummaryRow label="Nhóm thi" value={selectedCurriculum.examCategory || '—'} /><SummaryRow label="Đầu vào" value={selectedCurriculum.entryLevel || '—'} /><SummaryRow label="Mục tiêu" value={formatTarget(selectedCurriculum)} /><SummaryRow label="Cấu trúc" value={`${selectedCurriculum.totalUnits || 0} unit · ${selectedCurriculum.totalSessions || 0} buổi`} /><SummaryRow label="Trạng thái" value={selectedCurriculum.statusLabel || selectedCurriculum.status || '—'} /></div> : <p className="mt-4 text-sm leading-6 text-[#806765]">Chọn giáo trình để chương trình kế thừa Unit, học liệu, Practice, Module Test và Flashcard.</p>}
          </section>

          <button className={`${PRIMARY_BUTTON_CLASS} w-full justify-center`} disabled={saving} onClick={save} type="button"><Save className="h-4 w-4" />{saving ? 'Đang lưu...' : 'Lưu chương trình'}</button>
