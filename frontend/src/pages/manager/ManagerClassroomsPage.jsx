...
export default function ManagerClassroomsPage() {
  ...
  const saveClassroom = async (event) => {
    ...
    if (!editingId) {
      setMessage('Lớp mới phải được tạo từ đề xuất đã được Manager phê duyệt.');
      return;
    }
      ...
      {editorOpen ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/45 p-4 backdrop-blur-sm">
          ...
          <form className="relative z-10 flex max-h-[92vh] w-full max-w-5xl flex-col overflow-hidden rounded-3xl bg-white shadow-2xl" onSubmit={saveClassroom}>
            <div className="flex items-center justify-between border-b border-gray-100 px-6 py-5"><div><h3 className="font-['Manrope'] text-xl font-extrabold text-[#2b2828]">Chỉnh sửa thông tin vận hành</h3><p className="mt-1 text-xs text-[#8b706e]">Lớp đã được Manager duyệt; Staff chỉ cập nhật thông tin vận hành cần thiết.</p></div><button aria-label="Đóng" className="rounded-xl border border-gray-200 p-2 text-[#584140]" onClick={() => setEditorOpen(false)} type="button"><X className="h-5 w-5" /></button></div>
