...
class ViewPaymentHistoryTest {
    ...
    void listMyOrders_ReturnsHistoryWithOneOrder() {
        ...
        PaymentOrder paidOrder = PaymentOrder.builder()
                ...
                .student(learner)
                .courseIdsCsv("100,101")
                .courseTitles("IELTS Foundation|IELTS Advanced")
                .amount(500_000L)
    ...
    void getOrderStatus_ReturnsOrderDetail() {
        // Arrange
        String email = "learner@example.com";
        Long orderCode = 2026001L; // ├ính xß║í tß╗½ transaction_id = "TXN-2026-001"
        User learner = User.builder().id(1L).email(email).build();
    ...
    void listMyOrders_Pagination_ReturnsPageSlice() {
        ...
        // 3 ─æ╞ín h├áng thuß╗Öc learner_id=1, mß╗¢i nhß║Ñt tr╞░ß╗¢c
        PaymentOrder order1 = PaymentOrder.builder()
                .orderCode(1_700_000_000_001L).student(learner)
                .courseIdsCsv("100").courseTitles("IELTS Foundation")
                .amount(500_000L).originalAmount(500_000L)
                .status(PaymentOrderStatus.PAID)
                .createdAt(LocalDateTime.of(2026, 8, 3, 10, 0))
                .paidAt(LocalDateTime.of(2026, 8, 3, 10, 5))
                .build();
        PaymentOrder order2 = PaymentOrder.builder()
                .orderCode(1_700_000_000_002L).student(learner)
                .courseIdsCsv("101").courseTitles("IELTS Advanced")
                .amount(700_000L).originalAmount(700_000L)
                .status(PaymentOrderStatus.PAID)
                .createdAt(LocalDateTime.of(2026, 8, 2, 10, 0))
                .paidAt(LocalDateTime.of(2026, 8, 2, 10, 5))
                .build();
        PaymentOrder order3 = PaymentOrder.builder()
                .orderCode(1_700_000_000_003L).student(learner)
                .courseIdsCsv("102").courseTitles("IELTS Speaking")
                .amount(300_000L).originalAmount(300_000L)
                .status(PaymentOrderStatus.PAID)
        ...
        // Act - m├┤ phß╗Ång slice theo page=0, size=10 (service hiß╗çn ch╞░a hß╗ù trß╗ú ph├ón trang)
        ...
        assertThat(pageSlice).hasSize(3); // 3 ─æ╞ín ─æß╗üu nß║▒m trong trang 0
        assertThat(pageSlice.get(0).getOrderCode()).isEqualTo(1_700_000_000_001L); // mß╗¢i nhß║Ñt
        ...
        assertThat(totalPages).isEqualTo(1); // 3 bß║ún ghi / 10 mß╗ùi trang = 1 trang
    }
}
