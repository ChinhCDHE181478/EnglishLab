...
-- ---------------------------------------------------------------------------
-- V1 baseline created a partial UNIQUE INDEX uk_vocabulary_progress_student_course_term
-- on learner_progress_records. Rename it to a _legacy suffix BEFORE creating the
-- dedicated vocabulary_progress table below, otherwise PostgreSQL rejects the
-- inline CONSTRAINT with the same name (constraint and index share the relation
-- namespace). Run only when the legacy index still exists to keep V3 idempotent.
DO $$
BEGIN
    IF EXISTS (
        SELECT 1 FROM pg_class
        WHERE relkind IN ('i', 'I')
          AND relname = 'uk_vocabulary_progress_student_course_term'
          AND NOT EXISTS (
              SELECT 1 FROM pg_constraint WHERE conname = 'uk_vocabulary_progress_student_course_term'
          )
    ) THEN
        ALTER INDEX uk_vocabulary_progress_student_course_term
            RENAME TO uk_vocabulary_progress_student_course_term_legacy;
    END IF;
END $$;
